...
EtherNetwork <- R6Class("EtherNetwork",list(
  ...
  topInfo = NULL,
  nodesInfo = NULL,
  initialize = function(modelName, fileLoc, timeFormat) {
  ...
  filterPlayers = function(){
    ...
    #   periodDF <- subset(self$networkDF,time==cur)
    #   periodDF$type = "org"
    #   periodDFfix <- selectTop(periodDF,topsize = toprank)
    #   periodDFfix$type = "fix"
    #   periodDFpct <- selectTop(periodDF,topsize = 0.1*nrow(periodDF))
    #   periodDFpct$type = "pct"
    #   
    # }
    self$nodesInfo = map(self$periodList,function(x){
      periodDF <- subset(self$networkDF,time==x)
      periodDF$org = TRUE
      
      periodDFfix100 <- selectTop(periodDF,topsize = 100)
      periodDFfix100$top100 = TRUE
      periodDFfix150 <- selectTop(periodDF,topsize = 100)
      periodDFfix150$top150 = TRUE
      
      periodDFpct5 <- selectTop(periodDF,topsize = 0.05*nrow(periodDF))
      periodDFpct5$pct5 = TRUE
      periodDFpct10 <- selectTop(periodDF,topsize = 0.1*nrow(periodDF))
      periodDFpct10$pct10 = TRUE
      periodDFpct20 <- selectTop(periodDF,topsize = 0.2*nrow(periodDF))
      periodDFpct20$pct20 = TRUE
      res = reduce(list(periodDF,periodDFfix100,periodDFfix150,periodDFpct5,periodDFpct10,periodDFpct20),left_join,by=c("from","to","value"))
      return(res)
    })
  ...
  GUDHIRun = function(distFolder,outFolder){
    ...
    fileList <- list.files(distFolder,pattern = "csv$")
    for (f in fileList) {
      GUDHIpath = "C:/TDA/2018-01-31-09-25-53_GUDHI_2.1.0_WIN64_UTILS/rips_distance_matrix_persistence.exe"
...
getTopList <- function(dat,topsize=100){
