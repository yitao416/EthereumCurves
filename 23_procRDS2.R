...
########  functions ###########
measureFun <- function(pred,ref,para = "Sensitivity"){
  cf = caret::confusionMatrix(pred,ref,positive = "TRUE")
  cfmatrix = rbind(as.matrix(cf,what="overall"),
                   as.matrix(cf,what="classes"))
  value = cfmatrix[para,] %>% as.numeric()
...
measFlagsFun <- function(shortcut){
  ...
  accTokens = lapply(flags, function(flagCur){
    ...
    tmp = lapply(pdnames,function(v){
      group_var <- "token"   # group by this variable
      summ <- paste0('measureFun(', v,',',flagCur ,')')  # construct summary method, e.g. mean(mpg)
      summ_name <- paste0( v,shortcut)  # construct summary variable name, e.g. mean_mpg
...
senTokens = measFlagsFun("_sen")

########  functions ###########
measureFun <- function(pred,ref,para = "Specificity"){
  cf = caret::confusionMatrix(pred,ref,positive = "TRUE")
  cfmatrix = rbind(as.matrix(cf,what="overall"),
                   as.matrix(cf,what="classes"))
  value = cfmatrix[para,] %>% as.numeric()
...
specTokens = measFlagsFun("_spec")



save(list = c("accTokens","senTokens","specTokens"),file = "measures.Rdata")
