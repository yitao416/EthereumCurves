...
inputFolder = "02_output2/"
outputFolder = "03_joinBetti2/"
...
write_rds(bind_rows(rollDepthList),"rollDepth7_2.rds")
...
rollDF = read_rds("rollDepth7_2.rds")
...
## Check 
rollDF[,"token"] %>% unique()
tmp = rollDF %>% filter(token == "decentraland",betti =="B0") %>% head(10)
tmp

### output

write_rds(rollDF,"rollDepth7_tks_2.rds")
